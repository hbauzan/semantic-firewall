...
To: Senior IT Architect
From: Sovereign Semantic Architect (SSA)
Subject: Refactoring Auto-Calibration to "Non-Intrusive" Smart Defaults (Phase 2.1-B)
The previous auto-calibration logic was identified as "intrusive" because it prioritized hardcoded constants over explicit user intent. We are now implementing Non-Intrusive Smart Defaults. This ensures that calibration acts as a safety net when toggling modes, but yields immediately to manual slider adjustments or profile loads.
1. REFINED PROJECT DIRECTIVE (Translated & Optimized)
Role: Senior Backend Architect
Context: The auto-calibration in app/api/routes.py must be refactored to prevent overwriting deliberate user configurations.
Logic Requirements:
Respect User Intent: If the ConfigUpdate payload contains threshold values different from the current global state, these must be preserved.
Smart Fallbacks: Apply Phase 2.1 constants (Negative: Cos 0.6197, Exc 170; Positive: Cos 0.5315, Exc 150) ONLY if the firewall_mode is being toggled AND the thresholds in the request match the current state (indicating the user hasn't touched the sliders).
Profile Sovereignty: The /galaxy/profiles/load/{name} route must bypass all calibration logic to ensure profiles are loaded exactly as saved.
MANDATORY EXECUTION PROTOCOL FOR PROGRAMMER AGENT:
VERIFICATION REQUIREMENT: You are strictly mandated to verify the operational integrity of these changes. You must execute pytest backend/perform_tests.py. If any test fails—specifically test_pipeline_config_sync—you must diagnose the failure, refactor the implementation, and re-test iteratively until a 100% pass rate is achieved. Do not report completion until the code is functionally verified against the test suite.
2. TECHNICAL UPDATES
2.1 manifest.json
code
JSON
{
    "version": "v2.21.0",
    "active_features": {
        "non_intrusive_auto_calibration": true,
        "user_intent_preservation": true,
        "shannon_entropy_noise_filter": true,
        "profile_load_sovereignty": true
    }
}
2.2 backend/app/api/routes.py (Surgical Refactor of update_config)
code
Python
@router.post("/galaxy/config", dependencies=[Depends(verify_api_key)])
async def update_config(config: ConfigUpdate):
    ...
    try:
        async with _config_lock:
            ...
            # --- Non-Intrusive Smart Calibration Logic ---
            mode_toggled = req_data["firewall_mode"] != current.firewall_mode
            
            # Check if user explicitly changed thresholds in this request
            # (If req value != current value, user is moving a slider)
            cos_manual_change = req_data["cosine_threshold"] != current.cosine_threshold
            exc_manual_change = req_data["excitation_threshold"] != current.excitation_threshold
            
            # Apply defaults ONLY if mode is toggled AND user didn't touch the sliders
            if mode_toggled and not cos_manual_change and not exc_manual_change:
        ...
        # Auto-persist to _last_used
        ProfileManager.save_profile("_last_used", new_state)
        return {"status": "updated", "config": new_state.model_dump()}
    except ValueError:
        raise HTTPException(status_code=422, detail="Invalid configuration values")
2.3 backend/app/core/firewall.py (Entropy Implementation)
code
Python
@staticmethod
    def run_noise_filter(
        ...
        """Burst Detection: Shannon Entropy of the Query Vector (Corpus-Independent)."""
        # Normalize Q to a probability distribution
        abs_q = np.abs(q_arr)
        p = abs_q / (np.sum(abs_q) + 1e-9)
        # Calculate Shannon Entropy in bits
        entropy = float(-np.sum(p * np.log2(p + 1e-9)))
        
        # Low entropy (< 4.5) indicates collapsed/repetitive embedding (GCG attack)
        if entropy < cfg.global_noise_limit:
            return False, "noise", {"entropy": entropy, "limit": cfg.global_noise_limit}
        return True, "noise", {"entropy": entropy}
3. PRE-FLIGHT DIAGNOSTIC & TEST SUITE SYNC
To comply with the MANDATORY EXECUTION PROTOCOL, I have reviewed backend/perform_tests.py. The test test_pipeline_config_sync currently checks if values sent via POST are persisted.
Refinement for perform_tests.py:
I will add a specific validation to ensure that sending a mode toggle with a custom threshold preserves the custom threshold.
code
Python
...
SSA FINAL VERDICT: The logic is now surgically precise. It assists the user without overriding them.
[SYSTEM INITIALIZED - PROCEED TO CODE APPLICATION AND PYTEST EXECUTION]

*Edited relevant file*

### Planner Response

Now let me read the current [routes.py](cci:7://file:///Users/hbauzan/treepwood/semantic-firewall/backend/app/api/routes.py:0:0-0:0) update_config to see exactly what needs to change, then apply all file edits in parallel:

*Viewed [routes.py](file:///Users/hbauzan/treepwood/semantic-firewall/backend/app/api/routes.py) *

### Planner Response

Good, I have the full picture. Now let me also check the current auto-calibration tests to understand what needs updating:

*Viewed [perform_tests.py](file:///Users/hbauzan/treepwood/semantic-firewall/backend/perform_tests.py) *

### Planner Response

Now I have the full picture. The existing [test_auto_calibration_negative_mode](cci:1://file:///Users/hbauzan/treepwood/semantic-firewall/backend/perform_tests.py:834:0-858:56) sends `cosine_threshold=0.5315` and `excitation_threshold=150` — the same values that are already in the current state. With the new non-intrusive logic, since the request values match the current state (no manual change), auto-calibration **should** still fire. These existing tests remain valid.

For [test_auto_calibration_positive_mode](cci:1://file:///Users/hbauzan/treepwood/semantic-firewall/backend/perform_tests.py:861:0-892:6), it sends `cosine=0.6197` and `exc=170` from negative state which has `cosine=0.6197` and `exc=170` — again matching current state, so auto-calibration fires. Also valid.

Now let me apply all changes in parallel:
...
Now update the architecture spec and tests:
...
**48/48 tests pass** (28.51s). Let me update the walkthrough and notify:
