...
class GroqProvider(BaseProvider):
    async def stream_chat(self, model: str, messages: list, **kwargs) -> AsyncGenerator[str, None]:
        api_key = settings.groq_key_value
        if not api_key:
            yield "data: {\"choices\": [{\"delta\": {\"content\": \"🔴 [LLM ERROR] Missing Groq API Key\"}, \"finish_reason\": \"error\"}]}\n\n"
            yield "data: [DONE]\n\n"
            return
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(10.0, read=300.0)) as client:
            async with client.stream("POST", url, json=payload, headers=headers) as response:
                if response.status_code != 200:
                    ...
                    error_text = f"🔴 [LLM ERROR] Groq API Error ({response.status_code}): {response.text}"
